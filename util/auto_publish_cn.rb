...
CHANGELOG_CN="CHANGES.cn"
SVN_ADDR="http://code.taobao.org/svn/tengine/trunk/"
WEBSITE="http://tengine.taobao.org/"
TMP="/tmp/tengine-website/"


CHANGELOG_HTML="changelog_cn.html"
INDEX_HTML="index_cn.html"
DOWNLOAD_HTML="download_cn.html"
...
system("mkdir -p #{TMP}");
system("wget -q #{SVN_ADDR}#{CHANGELOG_CN} -O #{TMP}#{CHANGELOG_CN}");

changelog = open("#{TMP}#{CHANGELOG_CN}")
unless changelog
    puts "Can't open the #{CHANGELOG_CN} file."
    exit 1
end
...
index_news = "\s\s\s\s<li>[#{today}] <a href=\"download/tengine-#{base}.#{major}.#{minor}.tar.gz\">Tengine-#{base}.#{major}.#{minor}</a> 版本正式发布 (<a href=\"changelog_cn.html##{base}_#{major}_#{minor}\">变更列表</a>).</li>\n"
